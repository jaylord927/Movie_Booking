...
// Auto-cancel unpaid bookings after 3 hours and release seats
...
if ($auto_cancel_result->num_rows > 0) {
    ...
    try {
        while ($expired_booking = $auto_cancel_result->fetch_assoc()) {
            ...
            // Get seat numbers from booked_seats table
                        ...
                        try {
                            ...
                            $booking_stmt = $conn->prepare("
                                INSERT INTO tbl_booking (
                                    ...
                                    booking_fee, status, booking_reference, payment_status
                            ...
                            $seat_insert_stmt = $conn->prepare("
                                INSERT INTO booked_seats (booking_id, seat_number, seat_type, price)
                                VALUES (?, ?, ?, ?)
                            ");
                            
                            foreach ($seat_details as $seat) {
                                $seat_insert_stmt->bind_param("issd", $booking_id, $seat['number'], $seat['type'], $seat['price']);
                                if (!$seat_insert_stmt->execute()) {
                                    throw new Exception("Failed to insert booked seat: " . $seat_insert_stmt->error);
                                }
                            }
                            $seat_insert_stmt->close();
